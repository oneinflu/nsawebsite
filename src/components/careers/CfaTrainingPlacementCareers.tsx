    ...
    <section className="container mx-auto max-w-6xl px-4 py-8">
      <div className="rounded-3xl bg-white p-8 shadow-sm ring-1 ring-slate-200">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-sky-500 to-cyan-500 text-white flex items-center justify-center">♞</div>
          <div>
            <h2 className="text-2xl md:text-3xl font-bold text-slate-900">Training & Placement Support</h2>
            <p className="text-slate-600">From exam strategy to breaking into investment roles</p>
          </div>
        </div>
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-3">
          {items.map((i) => (
            <div key={i.title} className="rounded-2xl border border-slate-200 bg-white p-6">
              <div className={`h-12 w-12 rounded-xl bg-gradient-to-br ${i.color} text-white flex items-center justify-center`}>★</div>
        ...
        <div className="mt-8 rounded-2xl border border-slate-200 bg-slate-50 p-6">
          <p className="text-slate-700">
            &ldquo;The structured mentorship and mock reviews helped me move from a support analyst role to buy-side research. The CFA-aligned prep made a clear difference.&rdquo;
          </p>
          <p className="mt-2 text-sm text-slate-500">— Analyst, Asset Management (Singapore)</p>
